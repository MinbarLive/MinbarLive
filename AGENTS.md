...
| **Static mode is a box around the text, not a band** (2026-08-06) | The overlay takes the whole monitor there and the height slider is greyed out, because static draws one block sized to what was just said and a shorter band cut it off at both ends. Nothing is lost by it: with Transparent on, the backdrop is a per-line ribbon hugging the text (`gui/subtitle_window.py _ribbon_rects`), so a full-height window paints only as much as the text needs. The stored height is ignored, never overwritten — leaving static restores the operator's band |
