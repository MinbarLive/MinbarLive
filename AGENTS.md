...
| Integrated window style is Windows-only; `windowed` is the default (2026-07-25, PR #25) | X11 without a compositor ignores per-window alpha, so the dim backdrop renders solid black and borderless panels don't stack. Gated in `_use_integrated_windows()` |
