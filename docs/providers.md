...
If a selected provider lacks a capability, the app falls back to the highest-ranked provider (OpenAI → Gemini → Anthropic) that has a usable key, and logs a warning once (e.g. Claude users transcribe via Gemini or OpenAI — a Claude setup needs one of those keys too).
