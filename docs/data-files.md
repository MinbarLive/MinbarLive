...
| File                           | Purpose                                                                                     |
| ------------------------------ | ------------------------------------------------------------------------------------------- |
| `quran_embeddings_openai.npz`  | **What the app loads** (default): verse texts + L2-normalized float32 matrix, 6,054 × 3072 (OpenAI `text-embedding-3-large`) |
| `quran_embeddings_gemini.npz`  | Optional Gemini-space matrix (`gemini-embedding-001`); used only when the AI provider is Gemini and this file exists |
