...
def _build_general_user_prompt(
    ...
    - Translate the Source Text and nothing beyond it. NEVER re-render, restate or summarise a line from the Context to make a short fragment look like a complete sentence — the reader already has those lines on screen, so repeating them puts the same sentence up twice. A Source Text of one or two words yields a correspondingly brief translation, even when it completes a sentence the Context began. Say what the fragment says, in as many words as {target_lang} needs, and stop.
