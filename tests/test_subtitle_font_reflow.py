...
def test_source_font_size_changes_independently_on_the_live_surface(
    ...
    if mode == SUBTITLE_MODE_REALTIME:
        # Our live transcript line uses the full translation size (2026-07-09
        # s3 decision, deliberately kept over the source-styled variant), so a
        # source-size change leaves the live line size untouched.
        live_id = window._live_items[0][0]
        assert tkfont.Font(font=window.canvas.itemcget(live_id, "font")).cget(
            "size"
        ) == window.get_current_font_size()
...
def test_live_transcript_recolors_between_talking_and_settled(
    make_surface, live_text, expected_slant
):
    # Our live line follows the 2026-07-09 s3 styling (deliberately kept over
    # the source-styled variant): it uses the full translation size, shows the
    # muted source colour while the speaker talks, and turns to the primary
    # translation colour in place once the utterance settles.
    window = make_surface(SUBTITLE_MODE_REALTIME)
    window._source_font_size_base = 40.0
    window._update_font()
    window.set_source_text_color("#bb66dd")

    window.set_live_text(live_text, settled=False)
    live_id = window._live_items[0][0]
    talking_font = tkfont.Font(font=window.canvas.itemcget(live_id, "font"))
    assert window.canvas.itemcget(live_id, "fill") == "#bb66dd"
    assert talking_font.cget("slant") == expected_slant
    assert talking_font.cget("size") == window.get_current_font_size()

    window.set_live_text(live_text, settled=True)
    live_id = window._live_items[0][0]
    settled_font = tkfont.Font(font=window.canvas.itemcget(live_id, "font"))
    assert window.canvas.itemcget(live_id, "fill") == window._primary_text
    assert settled_font.cget("size") == window.get_current_font_size()
    assert settled_font.cget("slant") == expected_slant
